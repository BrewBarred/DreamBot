...
//    private JTextField taskNameInput, taskDescInput, manualTargetInput, consoleSearch;
//    private JComboBox<ActionType> actionCombo;
//    private JComboBox<String> nearbyTargetCombo;
//
//    // --- Labels ---
...
//    ));
//
//    public enum ActionType { FISH, CHOP, MINE, ATTACK, TALK_TO, USE, DROP, OPEN, SMELT, SMITH, COOK, BURY, PRAY_AT, PAY_TOLL, PAY_CHARTER, PICK_UP, EXAMINE, TRADE, BANK }
//
//    public DreamBotMenu2(AbstractScript script) {
//        this.script = script;
...
//        actionCombo = new JComboBox<>(ActionType.values()); manualTargetInput = new JTextField(10); nearbyTargetCombo = new JComboBox<>();
//        styleComp(actionCombo); styleComp(manualTargetInput); styleComp(nearbyTargetCombo);
//        nearbyTargetCombo.addMouseListener(new MouseAdapter() { @Override public void mousePressed(MouseEvent e) { populateNearby(); } });
//        g.gridx = 0; g.gridy = 0; left.add(new JLabel("Action:"), g); g.gridx = 1; left.add(actionCombo, g);
//        g.gridx = 0; g.gridy = 1; left.add(new JLabel("Target (Manual):"), g); g.gridx = 1; left.add(manualTargetInput, g);
//        g.gridx = 2; left.add(new JLabel(" OR Nearby:"), g); g.gridx = 3; left.add(nearbyTargetCombo, g);
//        JButton btnAddAct = createStyledBtn("ADD ACTION", ACCENT_BLOOD);
//        btnAddAct.addActionListener(e -> { String target = manualTargetInput.getText().isEmpty() ? (String)nearbyTargetCombo.getSelectedItem() : manualTargetInput.getText(); builderActionModel.addElement(new Action((ActionType)actionCombo.getSelectedItem(), target)); });
//        g.gridy = 2; g.gridwidth = 4; left.add(btnAddAct, g);
...
//        taskNameInput = new JTextField(); taskDescInput = new JTextField(); styleComp(taskNameInput); styleComp(taskDescInput);
//        JPanel commitPanel = new JPanel(new GridLayout(0, 1, 5, 5)); commitPanel.setOpaque(false);
//        commitPanel.add(new JLabel("Task Name:")); commitPanel.add(taskNameInput); commitPanel.add(new JLabel("Description:")); commitPanel.add(taskDescInput);
//        JButton btnCommit = createStyledBtn("COMMIT AS TASK", ACCENT_BLOOD);
//        btnCommit.addActionListener(e -> { List<Action> acts = new ArrayList<>(); for(int i=0; i<builderActionModel.size(); i++) acts.add(builderActionModel.get(i)); libraryModel.addElement(new Task(taskNameInput.getText(), taskDescInput.getText(), acts)); builderActionModel.clear(); });
//        right.add(commitPanel, BorderLayout.NORTH); right.add(new JScrollPane(bActions), BorderLayout.CENTER); right.add(btnCommit, BorderLayout.SOUTH);
...
//        consoleArea = new JTextArea(); consoleArea.setBackground(Color.BLACK); consoleArea.setForeground(new Color(0, 255, 0)); consoleArea.setFont(new Font("Consolas", Font.PLAIN, 12));
//        ((DefaultCaret)consoleArea.getCaret()).setUpdatePolicy(DefaultCaret.ALWAYS_UPDATE);
//        consoleSearch = new JTextField(); styleComp(consoleSearch);
...
//        JPanel t = new JPanel(new BorderLayout()); t.setOpaque(false); t.add(consoleSearch, BorderLayout.CENTER); t.add(btnCaptureToggle, BorderLayout.EAST);
//        p.add(t, BorderLayout.NORTH); p.add(new JScrollPane(consoleArea), BorderLayout.CENTER);
//        return p;
//    }
//
//    // --- LOGIC ---
...
//        updateAll();
//        if (isCaptureEnabled && script != null) {
//            // Logic to pull from Client logs if available
//        }
//    }
//
//    public void logToMenu(String msg) {
//        if (!isCaptureEnabled) return;
//        SwingUtilities.invokeLater(() -> {
//            String filter = consoleSearch.getText().toLowerCase();
//            if (filter.isEmpty() || msg.toLowerCase().contains(filter)) consoleArea.append(msg + "\n");
//        });
//    }
//
...
//        names.addAll(NPCs.all().stream().map(n -> n.getName()).collect(Collectors.toSet()));
//        names.addAll(GameObjects.all().stream().map(o -> o.getName()).collect(Collectors.toSet()));
//        nearbyTargetCombo.setModel(new DefaultComboBoxModel<>(names.toArray(new String[0])));
//    }
//
...
//        JPanel headerStats = new JPanel(new GridLayout(2, 1)); headerStats.setOpaque(false); headerStats.setBorder(new EmptyBorder(10, 20, 10, 10)); styleHeaderLabel(totalXpGainedLabel); styleHeaderLabel(totalLevelsGainedLabel);
...
//    public void setCurrentExecutionIndex(int index) { this.currentExecutionIndex = index; }
//
//    public void incrementExecutionIndex() {
//        if (currentExecutionIndex < queueModel.size() - 1) {
//            currentExecutionIndex++;
//        } else {
//            isScriptPaused = true; // Stop when list is done
...
//        Task(String n, String d, List<Action> a) { this.name = n; this.desc = d; this.actions = new ArrayList<>(a); }
//        Task(Task o) { this(o.name, o.desc, o.actions); }
//        String getEditableString() { StringBuilder sb = new StringBuilder("NAME:"+name+"\nDESC:"+desc+"\nACTIONS:\n"); for(Action a : actions) sb.append(a.toString()).append("\n"); return sb.toString(); }
//        void parseFromEditor(String text) { /* Logic to re-parse edited text */ }
//        @Override public String toString() { return name + " (" + actions.size() + ")"; }
//    }
//
//    public static class Action {
//        public ActionType type;
//        public String target;
//        Action(ActionType t, String target) { this.type = t; this.target = target; }
//
//        // This is the "brain" of the action
//        public boolean execute() {
//            switch (type) {
//                case FISH:
//                    NPC spot = NPCs.closest(target);
//                    return spot != null && spot.interact("Net"); // or "Harpoon"
//                case CHOP:
//                    GameObject tree = GameObjects.closest(target);
//                    return tree != null && tree.interact("Chop down");
//                case ATTACK:
//                    NPC monster = NPCs.closest(target);
//                    return monster != null && monster.interact("Attack");
//                case BANK:
//                    return org.dreambot.api.methods.container.impl.bank.Bank.open();
//                // Add more cases for each ActionType here...
//                default:
//                    Logger.log("Action " + type + " not yet implemented!");
//                    return false;
//            }
//        }
...
//            if (index == currentExecutionIndex) { l.setBackground(TAB_SELECTED); l.setForeground(Color.WHITE); l.setText("▶ " + l.getText()); }
//            return l;
//        }
//    }
//
...
//        SkillData(Skill s) { this.skill = s; this.startXP = Skills.getExperience(s); this.startLevel = Skills.getRealLevel(s); trackerPanel.setBackground(new Color(30, 30, 30)); TitledBorder b = BorderFactory.createTitledBorder(new LineBorder(BORDER_DIM), " " + s.name() + " "); b.setTitleColor(ACCENT_BLOOD); trackerPanel.setBorder(b); JLabel[] ls = {lblGained, lblPerHour, lblRemaining, lblActs, lblTTL, lblProj}; for (JLabel l : ls) { l.setForeground(TEXT_MAIN); l.setFont(new Font("Consolas", Font.PLAIN, 12)); trackerPanel.add(l); } }
//        void update(int curXp, int curLvl, long start, int ph) {
//            int curMin = Skills.getExperienceForLevel(curLvl), curMax = Skills.getExperienceForLevel(curLvl + 1); lblLevel.setText(String.valueOf(curLvl)); lblXpString.setText(String.format("%,d / %,d XP", curXp, curMax)); mainBar.setValue((int) (((double)(curXp - curMin) / Math.max(1, curMax - curMin)) * 100)); long elapsed = System.currentTimeMillis() - start; int xph = (int) (Math.max(0, curXp - startXP) / Math.max(0.0001, elapsed / 3600000.0)); int rem = Math.max(0, curMax - curXp);
