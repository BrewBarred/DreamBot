...
// * @version 7.0.0-Elite
// */
//public class DreamBotMenu_NiceTaskBuilder extends JFrame {
//
...
//    private final DefaultListModel<String> builderActionModel = new DefaultListModel<>(); // Actions inside the builder
//
...
//    private JTextField taskNameInput, taskDescInput, targetInput, consoleSearch;
...
//    private final JLabel lblCoords = new JLabel("-"), lblGameState = new JLabel("-"), lblMemberIcon = new JLabel();
...
//    };
//
//    static final Set<Skill> F2P_SKILLS = new HashSet<>(Arrays.asList(
...
//    ));
//
//    public DreamBotMenu_NiceTaskBuilder(AbstractScript script) {
//        this.script = script;
//        this.startTime = System.currentTimeMillis();
//        if (script != null) script.getScriptManager().pause();
//
...
//        libraryEditorArea.setBorder(new TitledBorder(new LineBorder(BORDER_DIM), "Task Editor", 0, 0, null, ACCENT_BLOOD));
//
...
//
//        JButton btnSave = createStyledBtn("Save Changes", new Color(100, 100, 0));
//        btnSave.addActionListener(e -> {
//            Task t = libraryList.getSelectedValue();
//            if (t != null) { t.parseFromEditor(libraryEditorArea.getText()); libraryList.repaint(); }
//        });
//
//        JButton btnDel = createStyledBtn("Delete", ACCENT_BLOOD);
//        btnDel.addActionListener(e -> {
//            if (libraryList.getSelectedValue() != null && confirm("Delete task forever?")) libraryModel.removeElement(libraryList.getSelectedValue());
//        });
//
//        btnRow.add(btnSave); btnRow.add(btnAdd); btnRow.add(btnDel);
//        editPanel.add(new JScrollPane(libraryEditorArea), BorderLayout.CENTER);
//        editPanel.add(btnRow, BorderLayout.SOUTH);
//
//        p.add(new JScrollPane(libraryList));
//        p.add(editPanel);
//        return p;
//    }
//
...
//
//        actionCombo = new JComboBox<>(new String[]{"FISH", "DROP", "USE", "EXAMINE", "TRADE", "ATTACK", "TALK_TO", "CHOP", "MINE", "BANK"});
//        targetInput = new JTextField(12);
//        styleComp(actionCombo); styleComp(targetInput);
//
//        g.gridx = 0; g.gridy = 0; left.add(new JLabel("Action:"), g);
//        g.gridx = 1; left.add(actionCombo, g);
//        g.gridx = 0; g.gridy = 1; left.add(new JLabel("Target:"), g);
//        g.gridx = 1; left.add(targetInput, g);
//
//        JButton btnAddAction = createStyledBtn("Add Action", ACCENT_BLOOD);
//        btnAddAction.addActionListener(e -> builderActionModel.addElement(actionCombo.getSelectedItem() + " -> " + targetInput.getText()));
//        g.gridx = 0; g.gridy = 2; g.gridwidth = 2; left.add(btnAddAction, g);
//
//        JPanel right = new JPanel(new BorderLayout(0, 10)); right.setOpaque(false);
//        JList<String> actionList = new JList<>(builderActionModel); styleJList(actionList);
//
//        JPanel savePanel = new JPanel(new GridLayout(2, 2, 5, 5)); savePanel.setOpaque(false);
//        taskNameInput = new JTextField(); taskDescInput = new JTextField();
//        styleComp(taskNameInput); styleComp(taskDescInput);
//        savePanel.add(new JLabel("Task Name:")); savePanel.add(taskNameInput);
//        savePanel.add(new JLabel("Task Description:")); savePanel.add(taskDescInput);
//
//        JButton btnCommit = createStyledBtn("COMMIT TO LIBRARY", ACCENT_BLOOD);
//        btnCommit.addActionListener(e -> {
//            List<String> actions = new ArrayList<>();
//            for(int i=0; i<builderActionModel.size(); i++) actions.add(builderActionModel.get(i));
//            libraryModel.addElement(new Task(taskNameInput.getText(), taskDescInput.getText(), actions));
//            builderActionModel.clear(); taskNameInput.setText("");
//        });
//
//        right.add(new JLabel("Current Action Sequence:"), BorderLayout.NORTH);
//        right.add(new JScrollPane(actionList), BorderLayout.CENTER);
//
//        JPanel south = new JPanel(new BorderLayout()); south.setOpaque(false);
//        south.add(savePanel, BorderLayout.CENTER); south.add(btnCommit, BorderLayout.SOUTH);
//        right.add(south, BorderLayout.SOUTH);
//
//        p.add(left, BorderLayout.WEST); p.add(right, BorderLayout.CENTER);
//        return p;
//    }
//
//    // --- OUTPUT TAB ---
//    private JPanel createOutputTab() {
//        JPanel p = new JPanel(new BorderLayout()); p.setBackground(BG_BASE);
//        consoleArea = new JTextArea();
//        consoleArea.setBackground(Color.BLACK); consoleArea.setForeground(new Color(0, 255, 0));
...
//        btnCaptureToggle.addActionListener(e -> { isCaptureEnabled = !isCaptureEnabled; btnCaptureToggle.setText("Capture: " + (isCaptureEnabled ? "ON" : "OFF")); });
//
//        p.add(btnCaptureToggle, BorderLayout.NORTH);
...
//        String name, desc;
//        List<String> actions;
//        Task(String n, String d, List<String> a) { this.name = n; this.desc = d; this.actions = new ArrayList<>(a); }
//        Task(Task o) { this(o.name, o.desc, o.actions); }
//
...
//            sb.append("ACTIONS:\n");
//            for(String s : actions) sb.append(s).append("\n");
//            return sb.toString();
//        }
//
//        void parseFromEditor(String text) {
//            String[] lines = text.split("\n");
//            actions.clear();
//            for(String line : lines) {
//                if(line.startsWith("NAME:")) name = line.substring(5);
//                else if(line.startsWith("DESC:")) desc = line.substring(5);
//                else if(!line.equals("ACTIONS:")) actions.add(line);
//            }
//        }
//        @Override public String toString() { return name + " (" + actions.size() + " actions)"; }
//    }
//
//    // --- ORIGINAL WORKING UI HELPERS (UNTOUCHED) ---
...
//    private boolean confirm(String m) { return JOptionPane.showConfirmDialog(this, m, "Warning", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION; }
//
//    // --- RESTORED ORIGINAL METHODS (COPY-PASTED FROM YOUR WORKING CODE) ---
...
//        controls.add(btnPlayPause); controls.add(btnStop); controls.add(btnInputToggle);
...
//        styleHeaderLabel(totalXpGainedLabel); styleHeaderLabel(totalLevelsGainedLabel);
//        headerStats.add(totalXpGainedLabel); headerStats.add(totalLevelsGainedLabel);
...
//    private void stopScript() { if (confirm("Kill script?")) { script.stop(); dispose(); } }
...
//        JPanel container = new JPanel(new BorderLayout()); container.setBackground(BG_BASE); container.setBorder(new EmptyBorder(20, 20, 20, 20));
//        JPanel content = new JPanel(new GridLayout(1, 2, 20, 0)); content.setBackground(BG_BASE);
//        JPanel login = createInfoCard("Login Details"); addInfoRow(login, "Username", lblUsername); addInfoRow(login, "Password", lblPassword); addInfoRow(login, "Identifier", lblAcctId); addInfoRow(login, "Acct Status", lblAcctStatus);
//        JPanel game = createInfoCard("World"); addInfoRow(game, "Character Name", lblCharName); addInfoRowWithIcon(game, "Membership", lblMemberText, lblMemberIcon); addInfoRow(game, "World", lblWorld); addInfoRow(game, "Coordinates", lblCoords); addInfoRow(game, "Game State", lblGameState);
//        content.add(login); content.add(game); container.add(content, BorderLayout.NORTH); return container;
//    }
//
...
//        JPanel container = new JPanel(new BorderLayout()); container.setBackground(BG_BASE); CardLayout cardLayout = new CardLayout(); JPanel contentPanel = new JPanel(cardLayout); contentPanel.setBackground(BG_BASE); contentPanel.setBorder(new EmptyBorder(20, 20, 20, 20));
//        contentPanel.add(createClientPanel(), "Client"); contentPanel.add(createActivitiesPanel(), "Activities"); contentPanel.add(createAudioPanel(), "Audio"); contentPanel.add(createChatPanel(), "Chat"); contentPanel.add(createDisplayPanel(), "Display"); contentPanel.add(createControlsPanel(), "Controls"); contentPanel.add(createWarningsPanel(), "Warnings");
//        JPanel menuPanel = new JPanel(new GridLayout(10, 1, 0, 2)); menuPanel.setPreferredSize(new Dimension(180, 0)); menuPanel.setBackground(PANEL_SURFACE); menuPanel.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, BORDER_DIM));
//        ButtonGroup btnGroup = new ButtonGroup(); String[] cats = {"Client", "Display", "Audio", "Chat", "Controls", "Activities", "Warnings"};
//        for (String cat : cats) { JToggleButton btn = createMenuButton(cat); btn.addActionListener(e -> cardLayout.show(contentPanel, cat)); btnGroup.add(btn); menuPanel.add(btn); if (cat.equals("Client")) btn.setSelected(true); }
//        container.add(menuPanel, BorderLayout.WEST); container.add(contentPanel, BorderLayout.CENTER); return container;
//    }
//
...
//        panel.add(new JScrollPane(grid), BorderLayout.CENTER); totalLevelLabel.setForeground(TEXT_MAIN); panel.add(totalLevelLabel, BorderLayout.SOUTH); return panel;
//    }
...
//        JPanel top = new JPanel(new BorderLayout()); top.setOpaque(false); JLabel icon = new JLabel(loadSkillIcon(data.skill));
//        data.lblLevel.setForeground(ACCENT_BLOOD); data.lblLevel.setFont(new Font("Arial", Font.BOLD, 18)); top.add(icon, BorderLayout.WEST); top.add(data.lblLevel, BorderLayout.EAST);
//        data.lblXpString.setForeground(TEXT_DIM); data.lblXpString.setFont(new Font("Monospaced", Font.PLAIN, 10));
//        data.mainBar.setForeground(ACCENT_BLOOD); data.mainBar.setBackground(Color.BLACK);
//        gbc.gridy = 0; tile.add(top, gbc); gbc.gridy = 1; tile.add(data.lblXpString, gbc); gbc.gridy = 2; tile.add(data.mainBar, gbc);
...
//        }
//
//        totalXpGainedLabel.setText("Total XP Gained: " + String.format("%,d", totalXPGained)); totalLevelsGainedLabel.setText("Total Levels Gained: " + totalLevelsGained); totalLevelLabel.setText(String.format("F2P Total: %d | P2P Total: %d", f2pTotal, p2pTotal));
//
//        if (Client.isLoggedIn()) {
//            lblUsername.setText(String.valueOf(Players.getLocal().getName()));
...
//            boolean isMember = Client.isMembers(); lblMemberText.setText(isMember ? P2P : "F2P");
