...
public class DreamBotMenu5 extends JFrame {
    ...
    private static final Set<Skill> F2P_SKILLS = new HashSet<>(Arrays.asList(
            ...
            Skill.HITPOINTS, Skill.CRAFTING, Skill.MINING, Skill.SMITHING, Skill.FISHING, Skill.COOKING,
            Skill.FIREMAKING, Skill.WOODCUTTING, Skill.RUNECRAFTING
    ));

    public DreamBotMenu5(AbstractScript script) {
        this.script = script;
        ...
        tBtn.addActionListener(e -> {
            ...
            tBtn.setText(sidePanel.isVisible() ? "<" : ">");
            revalidate();
        });
        ...
        new javax.swing.Timer(5000, e -> {
            if (mainTabs.getSelectedIndex() == 2) fetchDynamicTargets();
        }).start();
    }
    ...
    public void setCurrentExecutionIndex(int i) {
        ...
        if (taskQueueList != null) taskQueueList.repaint();
    }
    ...
    private JPanel createTaskListTab() {
        ...
        btnDel.addActionListener(e -> {
            int sel = taskQueueList.getSelectedIndex();
            if (sel != -1) queueModel.remove(sel);
        });
        bottomRow.add(btnDel);
    ...
    private JPanel createBuilderTab() {
        ...
        g.gridy = 0; left.add(new JLabel("TASK NAME:"), g);
        g.gridy = 1; left.add(taskNameInput, g);
        g.gridy = 2; left.add(new JLabel("DESCRIPTION:"), g);
        g.gridy = 3; left.add(taskDescInput, g);
        g.gridy = 4; left.add(new JLabel("STATUS FLAIR:"), g);
        g.gridy = 5; left.add(taskStatusInput, g);
        ...
        g.gridy = 6; g.insets = new Insets(20, 5, 5, 5);
        left.add(btnR, g);
        ...
        JList<Action> bActions = new JList<>(builderActionModel);
        styleJList(bActions);

        JButton btnAddLib = createStyledBtn("ADD TO LIBRARY", new Color(0, 100, 0));
        btnAddLib.addActionListener(e -> {
            List<Action> acts = new ArrayList<>();
            for (int i = 0; i < builderActionModel.size(); i++) acts.add(builderActionModel.get(i));
        ...
        center.add(new JScrollPane(bActions), BorderLayout.CENTER);
        center.add(btnAddLib, BorderLayout.SOUTH);
        ...
        btnAddAct.addActionListener(e -> {
            if (!manualTargetInput.getText().isEmpty()) {
                builderActionModel.addElement(new Action((ActionType) actionCombo.getSelectedItem(), manualTargetInput.getText()));
            }
        });
    ...
    private void fetchDynamicTargets() {
        SwingUtilities.invokeLater(() -> {
            ...
            if (type == ActionType.ATTACK) {
                names = NPCs.all().stream().filter(n -> n != null && n.hasAction("Attack")).map(n -> n.getName()).collect(Collectors.toSet());
            } else if (type == ActionType.CHOP) {
                names = GameObjects.all().stream().filter(o -> o != null && o.hasAction("Chop down")).map(o -> o.getName()).collect(Collectors.toSet());
            } else if (type == ActionType.MINE) {
                names = GameObjects.all().stream().filter(o -> o != null && o.hasAction("Mine")).map(o -> o.getName()).collect(Collectors.toSet());
            } else {
                names = NPCs.all().stream().filter(Objects::nonNull).map(n -> n.getName()).collect(Collectors.toSet());
            }
    ...
    public static class Task {
        public String name, desc, status;
        ...
        public boolean execute() {
            switch (type) {
                ...
                case BANK:
                    return Bank.open();
                case MINE:
                    GameObject rock = GameObjects.closest(target);
    ...
    // --- Restored Original Logic & Utilities ---
    ...
    private JPanel createLibraryTab() {
        ...
        btnAdd.addActionListener(e -> {
            if (libraryList.getSelectedValue() != null) queueModel.addElement(new Task(libraryList.getSelectedValue()));
        });
    ...
    private void initializeGenericLibrary() {
        libraryModel.addElement(new Task("Woodcutter", "Chops Trees", Arrays.asList(new Action(ActionType.CHOP, "Tree")), "Chopping..."));
    }
    ...
    private void stopScript() {
        if (JOptionPane.showConfirmDialog(this, "Stop Script?") == JOptionPane.YES_OPTION) {
            script.stop();
            dispose();
        }
    }
    ...
    private JPanel createOutputTab() {
        ...
        t.setOpaque(false);
        t.add(consoleSearch, BorderLayout.CENTER);
        t.add(btnCaptureToggle, BorderLayout.EAST);
    ...
    private void shiftQueue(int dir) {
        ...
        if (idx == -1 || idx + dir < 0 || idx + dir >= queueModel.size()) return;
        Task t = queueModel.remove(idx);
        queueModel.add(idx + dir, t);
        taskQueueList.setSelectedIndex(idx + dir);
    }

    private void styleComp(JComponent c) {
        c.setBackground(PANEL_SURFACE);
        c.setForeground(TEXT_MAIN);
        if (c instanceof JTextField) ((JTextField) c).setCaretColor(ACCENT_BLOOD);
    }
    ...
    private ImageIcon loadMiscIcon(String name) {
        try {
            return new ImageIcon(new ImageIcon(Objects.requireNonNull(getClass().getResource("/resources/icons/misc/" + name + ".png")))
    ...
    private ImageIcon loadSkillIcon(Skill skill) {
        try {
            return new ImageIcon(new ImageIcon(Objects.requireNonNull(getClass().getResource("/resources/icons/skills/" + skill.name().toLowerCase() + "_icon.png")))
    ...
    private ImageIcon loadStatusIcon(String name) {
        try {
            return new ImageIcon(new ImageIcon(Objects.requireNonNull(getClass().getResource("/resources/icons/status/" + name + ".png")))
    ...
    private JPanel createPlayerPanel() {
        ...
        content.add(login); content.add(game);
    ...
    private void addInfoRowWithIcon(JPanel p, String key, JLabel valLabel, JLabel iconLabel) {
        ...
        valLabel.setForeground(TEXT_MAIN);
        rightSide.add(valLabel); rightSide.add(iconLabel);
        row.add(k, BorderLayout.WEST);
    ...
    private JPanel createSettingsInterface() {
        ...
        String[] cats = {"Client", "Display", "Audio", "Chat", "Controls", "Activities", "Warnings"};
    ...
    private JPanel createSkillTile(SkillData data) {
        ...
        top.add(data.lblLevel, BorderLayout.EAST);
        ...
        gbc.gridy = 2; tile.add(data.mainBar, gbc);
